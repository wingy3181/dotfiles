...
show_spinner() {
    ...
    while kill -0 "$PID" &>/dev/null; do
        ...
        frameTextLength=${#frameText}

        if [ "$terminalWindowWidth" -lt "$frameTextLength" ]; then
            numberOfLinesToBeCleared=$(( numberOfLinesToBeCleared + ( frameTextLength / terminalWindowWidth ) ))
        fi
