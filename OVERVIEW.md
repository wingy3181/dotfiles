...
- [Safari Technology Preview](https://developer.apple.com/safari/technology-preview/) - Apple's web browser with newest Safari features and development tools
...
TO DO
- [X] folders within create_directories.sh
- [X] files/folders to symlink within create_symbolic_links.sh
- [X] java env (jenv)
- [X] Rupa's z
- [X] How autocompletion works?
      ...
      AFTER INSTALLING bash-completion2
...
/dotfiles/src/shell/macos/bash_autocomplete
  brew --prefix = /usr/local
  . "$(brew --prefix)/share/bash-completion/bash_completion"
    /usr/local/share/bash-completion/completions/*

/dotfiles/src/os/install/node_versions.sh = ~/.bash.local
  export NVM_DIR="/Users/cyip2/.nvm"
  source "$NVM_DIR/bash_completion"

/dotfiles/src/shell/bash_bash-it
  ~/.bash_it/bash_it.sh
    ~/.bash_it/completion/enabled/*.completion.bash

/dotfiles/src/os/install/jenv.sh = ~/.bash.local
  export JENV_DIR="/Users/cyip2/.jenv"
  if which jenv > /dev/null; then eval "$(jenv init -)"; fi
    ~/.jenv/libexec/jenv-init
      ~/.jenv/completions/*.bash
...
- [x] - start chrome no seccurity - alias chrome-insecure
- [X] npm packages to install globally within install_npm_packages.sh
- [X] node versions to install in nvm within install_node_versions.sh
- [X] Cherry-pick alrra latests changes
- [X] Change generation of public key to not delete and not make it for github only - over
- [X] Change and remove Ubuntu stuff
- [X] Change default dotfiles directory in setup.sh
- [X] Run brew --repository later and check config of .git/config to see if it contains homebrew.analyticsdisabled
      - Is /usr/local and yes it does
- [X] Use git from homebrew (like how they use bash from homebrew see change_default_bash_version.sh)
- [X] What happens if you brew install the same application twice or over the top of something installed manually?
It will override the manually installed applications
- [X] What happens if you brew tap a tap value twice?
If installed via brew already you have to force it,some applications will be self  updatingf installed via brew already you have to force it,some applications will be self  updating
- [X] Add documentation about the environment variables to debug and to restrict applications installed

Brew apps are installed to /usr/local/Cellar
  Some of the commands are symlink'd to /usr/local/bin or /usr/local/opt/
Cask apps are installed to /opt/homebrew-cask/Caskroom
  Some of the applications are symlnk'd to ~\Applications
  Manual apps are installed to \Applications which should be uninstalled
  # See https://github.com/caskroom/homebrew-cask/blob/master/lib/hbc/locations.rb
  # Specifically caskroom and appdir definitions
  # Can changes
  # https://github.com/caskroom/homebrew-cask/pull/21857 - Change default location of caskroom
  # https://github.com/caskroom/homebrew-cask/pull/13966 - No symlink
  # See https://github.com/caskroom/homebrew-cask/blob/master/doc/man_page/brew-cask.1.md for more info

> **TODO:**
> - [X] Chrome extensions : Just sync it using google account
> - [X] Atom and its extensions
> - [X] MGL set proxy - dotfiles-private/src/shell/bash_bash-it - set BASH_IT_HTTPS_PROXY/BASH_IT_HTTP_PROXY/BASH_IT_NO_PROXY
> - [X] npm version http://stackoverflow.com/questions/9755841/how-can-i-change-the-version-of-npm-using-nvm
>       - http://stackoverflow.com/questions/9755841/how-can-i-change-the-version-of-npm-using-nvm
>       - Seems to change versions as you switch versions using nvm (at least between npm v2 and v3 anyway)
> - [X] Local overrides for bash, git and vim using .local
>       Nothing to customize
> - [X] Do I need things in the bin directory? or customize my own things in here
>       https://github.com/wingy3181/dotfiles/issues/9
> - [X] http://stackoverflow.com/questions/31215452/intellij-idea-importing-gradle-project-getting-java-home-not-defined-yet
>       /Users/cyip2/Library/Preferences/IntelliJIdea2016.1/options/jdk.table.xml
        ~/.jenv/versions
> - [X] ack vs ag (silver searcher) - https://github.com/wingy3181/dotfiles/issues/1
> - [X] Docker https://github.com/wingy3181/dotfiles/issues/7
> - [X] Vagrant https://github.com/wingy3181/dotfiles/issues/8
> - [X] zsh and/or fishshell https://github.com/wingy3181/dotfiles/issues/4 and https://github.com/wingy3181/dotfiles/issues/5
> - [X] For zsh or fishshell, would we have to use bash version and add to /etc/shell (like for bash in change_default_bash_version.sh)
>       Yes we would
> - [X] Configure Alfred & iTerm2 -> https://github.com/wingy3181/dotfiles/issues/2 and https://github.com/wingy3181/dotfiles/issues/3
> - [X] Look into how vim has been setup with Vundle and plugins used
>       http://vimawesome.com/
>       http://vimcasts.org/episodes/
>          https://github.com/wingy3181/dotfiles/issues/6
> - [X] How to see current aliases, functions , etc
>       alias
...
> - [X] What else is useful in my functions? qh, deleting files and clearing sqllite (empty-trash)
...
>       git cc
>       git cm
> - [X] Fix use of less-pretty-cat breaking qh
> - [X] why grunt cli is not available in new terminal
>       ~/.nvm/versions/node/<version>/bin
>       switching versions of node using nvm means have to re-install globally packages
>       See https://github.com/creationix/nvm: Can use the reinstall-packages-from option
> - [X] npm global things installed?
> - [x] zopfli not installed unless i need it later...
> - [X] Configure XtraFinder
> - [X] Bartender 2?
> - [X] Computer name in set_ui_and_ux_preferences.sh
> - [X] Show current computer name and prompt to change
> - [X] Setup create directories
> - [X] Setup symbolic links for tmux, vim, atom
> - [X] Prompt install each homebrew application or Y to all
> - [X] Aliases in /usr/local/bin (e.g. wstorm)
> - [X] VIM?
> - [X] execute debug mode
> - [X] Libreoffice vs MsOffice (just reminder text)
> - [X] VirtualBox? or use inbuilt with docker? It's the same version
>       https://github.com/docker/toolbox/blob/master/Dockerfile.osx
>       https://github.com/caskroom/homebrew-cask/blob/master/Casks/virtualbox.rb   
>       For now decided to use Docker toolbox one
> - [X] Print txt file at end to tell user what else needs to be done manually
> - [X] Create PR for jenv to add remove all capability
> - [X] SDKman
> - [X] Synergy
> - [X] See https://github.com/Bash-it/bash-it / https://github.com/Homebrew/homebrew-completions
> - [X] Make travis ci build faster (caching or restricting application). Tried caching homeprew binaries..didn't work
>       
>
> **OS X customisations**
> - [X] Independent displays -> Mission Control -> Check 'Displays have separate spaces'
> - [X] Show Keychain Access menu in menubar (http://blog.taylormcgann.com/tag/cgsession/)
> - [X] Show Launchpad keyboard shortcut -> option-command-L
> - [X] Generation of SSH keys - too trivial command to script..just use ssh-keygen -t rsa -b 4096
...
> - [X] change of hostname, computername, etc
> - [X] Update git to not use apple one and use brew installed one
> - [X] Lock screen shell script and alias
> - [X] Signing commits using GPG - decided not to do for now
> - [X] Add cyip2 and Development to left hand of finder (just reminder text)
-->
